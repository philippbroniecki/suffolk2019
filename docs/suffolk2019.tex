...
One of the most common file types is \texttt{.csv} which means comma seperated values. Columns are seperated by commas and rows by line breaks.
...
To load the excel file, we can now use the \texttt{read\_excel()} function that is included in the \texttt{readxl} library. We need to provide the follwing arguments to the function:
...
In our example, columns are seperated by semicolons. We use the \texttt{read.table()} function and provide the following arguments:
...
sep & the symbol that seperates columns\tabularnewline
